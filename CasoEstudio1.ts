 ...
 * Todo en un solo archivo autosuficiente.
 */
...
class NodoLista<T> {
  public info: T;
  public sig: NodoLista<T> | null = null;
  constructor(info: T) {
    this.info = info;
  }
}
...
class ListaEnlazada<T> {
  ...
  public insertarFinal(elem: T): void {
  ...
  public obtenerEn(pos: number): T | null {
  ...
  public existe(predicado: (elem: T) => boolean): boolean {
    let aux = this.cabeza;
    while (aux !== null) {
  ...
  public buscar(predicado: (elem: T) => boolean): T | null {
    let aux = this.cabeza;
    while (aux !== null) {
...
class ColaCircular<T> {
  ...
  constructor(capacidad: number) {
    ...
    this.arreglo = new Array<T | null>(capacidad).fill(null);
  }
...
class NodoPila<T> {
  public info: T;
  public sig: NodoPila<T> | null = null;
  constructor(info: T) {
    this.info = info;
  }
}
...
class PilaEncadenada<T> {
  private tope: NodoPila<T> | null = null;
  private tam: number = 0;

  public apilar(elem: T): void {
  ...
  /**
   * Recorre la pila del TOPE hacia la BASE.
   */
  public aArrayDesdeTope(): T[] {
    const arr: T[] = [];
    let aux = this.tope;
    while (aux !== null) {
      arr.push(aux.info);
...
// ============================================================================
// 2. DOMINIO DEL PROBLEMA Y MODELOS
// ============================================================================

type Canal = 'MOSTRADOR' | 'DOMICILIO';
type EstadoPedido = 'EN_COLA' | 'EN_ARMADO' | 'LISTO' | 'CANCELADO';

interface Producto {
  codigo: string;
  nombre: string;
  receta: ListaEnlazada<string>; // Capas de base a tapa
}
...
interface Pedido {
  ...
  sin: ListaEnlazada<string>;
  minutoLlegada: number;
  estado: EstadoPedido;
  minutoFinalizacion?: number;
  cumplePromesa?: boolean;
}

interface RegistroDesperdicio {
  capa: string;
  motivo: 'CANCELADO' | 'DESAPILADO_CORRECCION';
}

// ============================================================================
// 3. LÓGICA DE NEGOCIO (SISTEMA DE COCINA)
// ============================================================================

class SistemaCocina {
  public catálogo: ListaEnlazada<Producto> = new ListaEnlazada<Producto>();
  public colaMostrador: ColaCircular<Pedido>;
  public colaDomicilio: ColaCircular<Pedido>;
  public pilaArmado: PilaEncadenada<string> = new PilaEncadenada<string>();
  
  public pedidoEnArmado: Pedido | null = null;
  public contadorMostradorAtendidos: number = 0; // Regla R2 (3:1)
  private consecutivoPedido: number = 1;

  // Métricas
  public totalAtendidosMostrador: number = 0;
  public totalAtendidosDomicilio: number = 0;
  public totalRechazadosMostrador: number = 0;
  public totalRechazadosDomicilio: number = 0;
  public erroresArmadoContador: number = 0;
  public listaDesperdicios: ListaEnlazada<RegistroDesperdicio> = new ListaEnlazada<RegistroDesperdicio>();
  public historialPedidos: ListaEnlazada<Pedido> = new ListaEnlazada<Pedido>();
  public bitacora: ListaEnlazada<string> = new ListaEnlazada<string>();

  constructor(capacidadMaxColas: number = 5) {
    this.colaMostrador = new ColaCircular<Pedido>(capacidadMaxColas);
    this.colaDomicilio = new ColaCircular<Pedido>(capacidadMaxColas);
    this.registrarBitacora("Sistema inicializado. Colas configuradas con capacidad C=" + capacidadMaxColas);
  }

  public registrarBitacora(msj: string): void {
    const hora = new Date().toLocaleTimeString();
    this.bitacora.insertarFinal(`[${hora}] ${msj}`);
  }

  // RF-01: cargarCatalogo(datos)
  public cargarCatalogo(productos: { codigo: string; nombre: string; receta: string[] }[]): void {
    for (const p of productos) {
      ...
      const recetaLista = new ListaEnlazada<string>();
  ...
  public registrarPedido(canal: Canal, codigoProd: string, sinCapas: string[], minuto: number): Pedido {
    ...
    const sinLista = new ListaEnlazada<string>();
    ...
    // Aplicar R7: Limite de capacidad en colas
    if (canal === 'MOSTRADOR') {
  ...
  public siguientePedido(): { pedido: Pedido; motivo: string } | null {
    ...
    if (elegido) {
      ...
      this.pilaArmado.vaciar(); // Limpiar estación
  ...
  // Obtener la receta esperada descontando las capas que el cliente pidió omitir (sin)
  public obtenerRecetaEsperada(pedido: Pedido): string[] {
    ...
    // Filtra las capas omitidas por el cliente
    return recetaOriginal.filter(capa => !sinArray.includes(capa));
  }

  // RF-04: colocarCapa(capa)
  public colocarCapa(capa: string): { exitoso: boolean; advertenciaR5?: string } {
    ...
    // El paso esperado depende de cuántas capas ya están en la pila
  ...
  /**
   * RF-06: verificar()
   * Explicación de Inversión de Sentido:
   * La receta se define de la BASE (índice 0) a la TAPA (último índice).
   * La pila se recorre del TOPE (última capa colocada) a la BASE.
   * Para comparar sin destruir la pila:
   *  1. Convertimos la pila a arreglo [Tope -> Base].
   *  2. Lo invertimos para obtener [Base -> Tope].
   *  3. Se compara elemento a elemento con la receta esperada.
   */
  public verificar(): { esCorrecto: boolean; faltantes: string[]; sobrantes: string[]; colocadas: string[] } {
    if (!this.pedidoEnArmado) throw new Error("ERR: No hay pedido en armado.");

    const recetaEsperada = this.obtenerRecetaEsperada(this.pedidoEnArmado);
    const pilaDesdeTope = this.pilaArmado.aArrayDesdeTope();
    
    // Invertir sentido (LIFO a FIFO)
    ...
    return {
      ...
      faltantes,
      sobrantes,
      colocadas: pilaDesdeBase
    };
  }

  // RF-07: marcarListo()
  public marcarListo(minutoActual: number): void {
    ...
    // Regla R8: Cumplimiento de tiempo promesa
  ...
  public cancelarPedido(): void {
    ...
    // Regla R6: Vaciar la pila y registrar como desperdicio
...
// ============================================================================
// 4. INTERFAZ GRÁFICA INTERACTIVA Y RENDERIZADO DOM
// ============================================================================

class InterfazCocina {
  private sistema: SistemaCocina;
  private minutoSimulado: number = 0;
  ...
  private inyectarEstilosCSS(): void {
    ...
    style.textContent = `
      ...
      * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', system-ui, sans-serif; }
      ...
      /* Visualizacion Pila Vertical (Estacion Armado) */
      ...
      .capa-block.error { background: #7f1d1d; border-color: var(--accent-red); }
  ...
  private construirEstructuraDOM(): void {
  ...
  private asociarEventosUI(): void {
    ...
    document.getElementById('btn-avanzar-minuto')?.addEventListener('click', () => {
      this.minutoSimulado++;
      (document.getElementById('lbl-minuto') as HTMLElement).innerText = this.minutoSimulado.toString();
    });
    ...
    document.getElementById('form-pedido')?.addEventListener('submit', (e) => {
      e.preventDefault();
      try {
        const canal = (document.getElementById('sel-canal') as HTMLSelectElement).value as Canal;
        const prodCodigo = (document.getElementById('sel-producto') as HTMLSelectElement).value;
        
        const capasOmitir: string[] = [];
        document.querySelectorAll('.chk-omitir:checked').forEach(chk => {
          capasOmitir.push((chk as HTMLInputElement).value);
        });
    ...
    // Selección de producto en formulario para desplegar sus capas omitibles
    document.getElementById('sel-producto')?.addEventListener('change', (e) => {
      this.renderizarOpcionesSin((e.target as HTMLSelectElement).value);
    });

    // Llamar Siguiente Pedido
    document.getElementById('btn-siguiente-pedido')?.addEventListener('click', () => {
      try {
        ...
        if (!res) {
          ...
          this.ocultarError();
          this.actualizarPantalla();
          // Auto navegar a Estación de Armado
          (document.querySelector('[data-tab="sec-armado"]') as HTMLElement)?.click();
        }
      } catch (err: any) {
        this.mostrarError(err.message);
      }
    ...
    document.getElementById('btn-colocar-capa')?.addEventListener('click', () => {
      try {
        const capa = (document.getElementById('sel-capa-colocar') as HTMLSelectElement).value;
  ...
  private actualizarSelectProductos(): void {
    const sel = document.getElementById('sel-producto') as HTMLSelectElement;
    sel.innerHTML = '';
  ...
  private renderizarOpcionesSin(codigoProd: string): void {
    const box = document.getElementById('box-capas-omitir') as HTMLElement;
    box.innerHTML = '';
  ...
  private mostrarError(msj: string): void {
    const el = document.getElementById('alert-global') as HTMLElement;
    el.innerText = msj;
  ...
  private actualizarPantalla(): void {
    // 1. Render Catálogo
    const catUI = document.getElementById('lista-catalogo') as HTMLElement;
    catUI.innerHTML = '';
    ...
    const colaMUI = document.getElementById('cola-mostrador-ui') as HTMLElement;
    ...
    (document.getElementById('cant-mostrador') as HTMLElement).innerText = `${this.sistema.colaMostrador.obtenerLongitud()} / 5`;

    const colaD = this.sistema.colaDomicilio.aArrayEnOrden();
    const colaDUI = document.getElementById('cola-domicilio-ui') as HTMLElement;
    ...
    (document.getElementById('cant-domicilio') as HTMLElement).innerText = `${this.sistema.colaDomicilio.obtenerLongitud()} / 5`;

    (document.getElementById('lbl-r2-counter') as HTMLElement).innerText = `${this.sistema.contadorMostradorAtendidos} / 3`;

    // 3. Render Estación de Armado
    const pedActivo = this.sistema.pedidoEnArmado;
    const infoActivo = document.getElementById('info-pedido-activo') as HTMLElement;
    const recetaExpUI = document.getElementById('receta-esperada-ui') as HTMLElement;
    const selCapa = document.getElementById('sel-capa-colocar') as HTMLSelectElement;
    ...
    // Render Pila (LIFO)
    const pilaUI = document.getElementById('pila-ui') as HTMLElement;
    pilaUI.innerHTML = '';
    const capasPila = this.sistema.pilaArmado.aArrayDesdeTope(); // [Tope -> Base]
    ...
    // 4. Render Métricas y Bitácora
    const metricsUI = document.getElementById('metrics-ui') as HTMLElement;
    ...
    const bitacoraUI = document.getElementById('bitacora-ui') as HTMLElement;
